-- @description Select next region in Region Manager and play it
-- @version 1.0
-- @author Codex
-- @about
--   Moves the Region/Marker Manager selection to the next region and starts it.
--   Playback always begins at the region start and stops automatically at the
--   region end. If no region is selected, the first region is used.

local EXT_SECTION = "JS_SelectNextRegionAndPlay"
...
local function main()
  ...
  if not list then
    reaper.ShowMessageBox(err, "Naechste Region konnte nicht gestartet werden", 0)
    return
  end
  ...
  if #region_rows == 0 then
    reaper.ShowMessageBox(
      ...
      "Naechste Region konnte nicht gestartet werden",
      0
    )
    return
  end
  ...
  local target = region_rows[1]

  if selected_row then
    for i = 1, #region_rows do
  ...
  if not region_start then
    reaper.ShowMessageBox(
      "Die naechste Region wurde im Projekt nicht gefunden.",
      "Naechste Region konnte nicht gestartet werden",
      0
    )
    return
  ...
  reaper.OnPlayButton()
  reaper.Undo_EndBlock("Select next region in Region Manager and play it", -1)
