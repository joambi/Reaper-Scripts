-- @description Play selected region in Region Manager from current position and stop at region end
-- @version 1.0
-- @author Codex
-- @about
--   Select a region in the Region/Marker Manager and run this script.
--   Playback starts from the current edit cursor position if it lies within
--   the selected region, otherwise from the region start. Playback stops
--   automatically at the region end.

local EXT_SECTION = "JS_PlaySelectedRegionFromCurrentPos"
...
local function main()
  ...
  local play_start = region_start
  if cursor_pos >= region_start and cursor_pos < region_end then
    play_start = cursor_pos
  end
  ...
  reaper.SetEditCurPos2(project, play_start, false, false)
  reaper.OnPlayButton()
  reaper.Undo_EndBlock("Play selected region in Region Manager from current position and stop at region end", -1)
