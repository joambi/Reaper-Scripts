-- @description JAR Post-Render B-Format-Analyse
-- @version 1.0
-- @author Codex
-- @about
--   Analysiert ein selektiertes B-Format-/HOA-Item oder eine frisch gerenderte B-Format-Datei
--   ueber analyze_bformat.py und erzeugt Analysedateien sowie optionale Vergleichs-SVGs.
...
--
-- Pfadlogik:
--   1. analyze_bformat.py direkt neben diesem Script
--   2. analyze_bformat.py in ./media/examples neben diesem Script
--   3. analyze_bformat.py im JAR-Ambisonics-Paper Vault unter dem Home-Verzeichnis

local script_name = "JAR Post-Render B-Format-Analyse"
local MIN_BFORMAT_CHANNELS = 4
...
local function get_script_dir()
  ...
  return source:match("(.+)/[^/]+$") or nil
end

local function resolve_analyze_script()
  local script_dir = get_script_dir()
  local home_dir = get_home_dir()
  local candidates = {}

  if script_dir then
    candidates[#candidates + 1] = script_dir .. "/analyze_bformat.py"
    candidates[#candidates + 1] = script_dir .. "/media/examples/analyze_bformat.py"
  end

  if home_dir then
    candidates[#candidates + 1] = home_dir .. "/OBSIDIAN/JAR-Ambisonics-Paper/media/examples/analyze_bformat.py"
    candidates[#candidates + 1] = home_dir .. "/Documents/OBSIDIAN/JAR-Ambisonics-Paper/media/examples/analyze_bformat.py"
  end
...
local function run_analysis()
  local ANALYZE_SCRIPT, analyze_candidates = resolve_analyze_script()
  ...
  if not file_exists(ANALYZE_SCRIPT) then
    local details = ""
    if analyze_candidates and #analyze_candidates > 0 then
      details = "\n\nGesucht in:\n  - " .. table.concat(analyze_candidates, "\n  - ")
    end
    reaper.ShowMessageBox(
      "analyze_bformat.py nicht gefunden.\n" ..
      "Erwarteter Pfad:\n" .. tostring(ANALYZE_SCRIPT) .. details .. "\n\n" ..
      "Lege analyze_bformat.py neben dieses Lua-Script oder in den JAR-Ambisonics-Paper Vault.",
      script_name .. " - Fehler", 0
    )
    return false
  end
